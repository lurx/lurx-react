...
describe('DrawerHeader', () => {
	...
	it('renders a string title when provided', () => {
		render(<DrawerHeader onClose={jest.fn()} title="My Drawer" />);
	...
	it('does not render a title container when title is not provided', () => {
		const { container } = render(<DrawerHeader onClose={jest.fn()} />);
	...
	it('calls onClose multiple times when clicked multiple times', () => {
		const onClose = jest.fn();
		render(<DrawerHeader onClose={onClose} />);
		fireEvent.click(screen.getByLabelText('Close drawer'));
		fireEvent.click(screen.getByLabelText('Close drawer'));
