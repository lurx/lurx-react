...
Instead, React attached a single event listener to the **root of your app** — the DOM node you passed to `createRoot`. Every click, every keypress, every input event in your entire application is caught by that one listener, and React figures out which component handler to call.
...
## Why Not Just Use Native Events?
...
**Consistency across browsers.** When React was created in 2013, browser event behavior was inconsistent. `onChange` didn't fire the same way in IE. Focus events bubbled in some browsers and not others. React's synthetic event layer normalized these differences — you got the same behavior everywhere.
...
**Batching.** In [Part 7](/blog/react-internals-7-state-updates-and-lanes), we'll see how React batches multiple `setState` calls into one render. This works because React controls when event handlers execute — it wraps them in a context that enables batching. With native listeners, React wouldn't have that control.
...
2. The event bubbles up through the DOM — past `<div>`s, past `<main>` — until it reaches your app's root node.
...
From there, React walks the `return` pointers (each fiber's parent) up the fiber tree, collecting every `onClick`, `onClickCapture`, etc. registered along the path. This simulates event propagation — but through the *fiber tree*, not the DOM tree.
...
This is by design — it means event bubbling follows the *component* hierarchy (which is your mental model) rather than the *DOM* hierarchy (which is an implementation detail). But it surprises developers who mix React and native event listeners.

---

## Synthetic Events

The `e` you get in your event handler isn't the browser's native event object. It's a **synthetic event** — a React wrapper that looks and behaves like a native event but is controlled by React.

For the most part, you'll never notice the difference. `e.target`, `e.preventDefault()`, `e.stopPropagation()` — they all work as expected. But there are a couple of things worth knowing:
...
## onChange: The Event That React Invented
...
Here's something you've experienced but probably never thought about: `setState` inside an `onClick` feels instant, but `setState` inside a `mousemove` can feel slightly laggy. That's not your imagination — React is deliberately treating them differently.
...
A click is a deliberate action — the user expects an immediate response. A mouse movement fires dozens of times per second — React can afford to batch those. The event system is making this priority decision *before your handler even runs*, so the right lane is already set when you call `setState`.
...
What happens when you click the button? It depends on the React version, the event type, and whether React used capture or bubble phase for its root listener. The exact ordering is genuinely hard to predict — and that's the point.
...
- **Propagation follows the fiber tree** — which matters for portals, where the fiber tree and DOM tree diverge.
