...
## JSX isn't HTML. It's a function call.
...
This comes from [`react/jsx-runtime`](https://github.com/facebook/react/blob/v19.2.8/packages/react/src/jsx/ReactJSXElement.js), a module you never import explicitly but your bundler resolves automatically. The `jsx()` function doesn't create a DOM node. It creates a **React element**, which is a plain JavaScript object:
...
## The element tree is a description, not a reality
...
The commit phase is where [`commitRoot`](https://github.com/facebook/react/blob/v19.2.8/packages/react-reconciler/src/ReactFiberCommitWork.js) runs. It walks the list of changes collected during the render phase and applies them.
...
Let that sink in. The part of React that most developers obsess over, "my component re-rendered!", is the cheap part. The DOM work is the expensive part, and React works hard to minimize it.
...
Every component marked *re-rendered* had its function called, which is the render phase. But only the one marked *text changed* produced different output. The commit phase touches one text node. Everything else is skipped.

So the next time someone on your team says "this component re-renders too much", ask them whether it re-renders or causes DOM mutations. Those are very different problems with very different solutions.
...
When `count` changes, `App` re-renders. But `items` is memoized, the same reference as before, so `ExpensiveList` bails out entirely. React never calls its function. The render phase skips that entire branch.
...
## Strict Mode, and why it makes sense now
...
We've covered a lot of ground. Let's tie it together by tracing a single update from click to pixels.
...
Steps 3 to 5 are the render phase: pure computation, no DOM. Step 6 is the commit phase, synchronous DOM mutation. Everything after is post-commit work.
