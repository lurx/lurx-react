...
`heavyFilter` takes 200ms. Without `startTransition`, every keystroke freezes the input for 200ms while React renders the new list. With it, the input stays responsive. You can type as fast as you want, and the list updates when React finds time.
...
Tasks go into a min-heap sorted by expiration time. When the browser is idle, the scheduler pops tasks off the heap and runs them. The key part is that it checks between tasks whether it should yield.

### The MessageChannel trick

How does the scheduler "yield to the browser"? It can't use `setTimeout(fn, 0)`, because browsers clamp that to a minimum of 4ms, wasting time. It can't use `requestAnimationFrame`, which is tied to the display refresh rate and fires before paint, not after.

Instead, the scheduler uses a [`MessageChannel`](https://github.com/facebook/react/blob/v19.2.8/packages/scheduler/src/forks/Scheduler.js):
...
Five lines of code. That's how React implements "cooperative multitasking", a concept that sounds like it belongs in an operating systems textbook, built on a browser API designed for iframe communication. Sometimes the cleverest solutions are the most unexpected.

---

## The work loop and shouldYield
...
[`shouldYieldToHost`](https://github.com/facebook/react/blob/v19.2.8/packages/scheduler/src/forks/Scheduler.js) checks how much time has elapsed since React started its current work slice:

```js
// Simplified from Scheduler.js; frameYieldMs is declared in SchedulerFeatureFlags.js
...
That interval comes from [`SchedulerFeatureFlags.js`](https://github.com/facebook/react/blob/v19.2.8/packages/scheduler/src/SchedulerFeatureFlags.js), where `frameYieldMs` is set to 5, and the scheduler can adjust it at runtime for higher-refresh displays. Every ~5ms, React checks: "Have I been hogging the main thread?" If yes, it stops the work loop, lets the browser handle any pending events and paint, and resumes later via the MessageChannel.

Because the fiber tree stores the current position in `workInProgress` (not on the call stack), React can stop at *any* fiber and resume from exactly that point. The tree is its own bookmark, which was the entire purpose of the Fiber architecture.
...
5. React processes the sync update immediately, so the input is updated.
...
## Suspense pauses a render by throwing a promise
...
React's render phase catches this thrown promise in [`throwException`](https://github.com/facebook/react/blob/v19.2.8/packages/react-reconciler/src/ReactFiberThrow.js):

```js
// Simplified from throwException
function throwException(fiber, thrownValue) {
  if (typeof thrownValue?.then === 'function') {
    // It's a thenable, so this is a Suspense suspension
...
## useDeferredValue, where Suspense meets transitions
...
You can think of it as `startTransition` but with a built-in "keep showing the old version while the new one loads" behavior. Under the hood, it's wired into the reconciler directly, with no extra state or effects needed.
...
Every abstraction React gives you, `useState`, `useEffect`, `Suspense`, `startTransition`, is a thin layer over a mechanical system of linked lists, bitmasks, and iterative tree traversal. Once you see the machinery, the behavior stops being surprising.
