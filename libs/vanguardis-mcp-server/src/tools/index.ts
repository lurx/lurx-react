...
export class VanguardisTools {
	...
	constructor() {
		...
		if (!fs.existsSync(this.vanguardisPath)) {
			this.vanguardisPath = path.resolve(
				process.cwd(),
				'../../libs/vanguardis',
			);
		}
		...
		if (!fs.existsSync(this.vanguardisPath)) {
			throw new Error(
				'Vanguardis library not found. Please ensure the MCP server is run from the correct location.',
			);
		}
	}
	...
	async listComponents(category?: string): Promise<string> {
		try {
			...
			const componentsList = filteredComponents
				.map(comp => {
					return `## ${comp.name}
...
${
	comp.props.length > 0
		? `- **Props**: ${comp.props.length} available`
		: '- **Props**: None'
}
			...
			const component = components.find(
				comp =>
					...
					comp.name
						.toLowerCase()
						.replace(/([A-Z])/g, '-$1')
						.slice(1) === componentName.toLowerCase(),
			);
...
`;
			...
			// Add usage example
			if (includeExample) {
				const example = await this.generateComponentExample(
					componentName,
					{},
					true,
					true,
	...
	async getDesignTokens(category = 'all'): Promise<string> {
		try {
			const designTokensPath = path.join(
				this.vanguardisPath,
				'src/types/design-system.types.ts',
			);
			...
			let result = `# Vanguardis Design Tokens${
...
`;
			...
			const componentUsage = `<${component.name}${propsString}${
				this.isVoidElement(component.name)
					? ' />'
					: `>
  {/* Component content */}
</${component.name}>`
...
**Usage**: Use \`vanguardis_get_hooks\` with a specific hook name to get detailed information.`;
	...
	async getUtils(utilName?: string): Promise<string> {
		try {
			...
			if (utilName) {
				const util = utils.find(
					u => u.name.toLowerCase() === utilName.toLowerCase(),
				);
				if (!util) {
...
**Usage**: Use \`vanguardis_get_utils\` with a specific utility name to get detailed information.`;
		...
		try {
			...
			if (type === 'all' || type === 'components') {
				...
				if (matchingComponents.length > 0) {
					results.push(
						`## Components (${matchingComponents.length} found)\\n` +
							matchingComponents
								.map(
									comp =>
			...
			if (type === 'all' || type === 'hooks') {
				...
				if (matchingHooks.length > 0) {
					results.push(
						`## Hooks (${matchingHooks.length} found)\\n` +
							matchingHooks
			...
			if (type === 'all' || type === 'utils') {
				const utils = await this.getUtilsInfo();
				const matchingUtils = utils.filter(
					util =>
				...
				if (matchingUtils.length > 0) {
					results.push(
						`## Utilities (${matchingUtils.length} found)\\n` +
							matchingUtils
			...
			if (results.length === 0) {
				return `No results found for query: "${query}" in ${
	...
	private parseComponent(
		...
		const testPath = filePath
			...
			.replace('src/components/', 'src/components/__tests__/');
		...
		const props = propsInterfaceMatch
			? this.parseProps(propsInterfaceMatch[1])
			: [];
	...
	private parseHooks(content: string, filePath: string): HookInfo[] {
		...
		for (const match of hookMatches) {
			...
			const hookPattern = new RegExp(
				`/\\*\\*[\\s\\S]*?\\*/[\\s\\S]*?export\\s+(?:function\\s+)?${hookName}`,
			);
			const hookMatch = content.match(hookPattern);
			const description = hookMatch
				? this.extractDescription(hookMatch[0])
				: `${hookName} hook`;
	...
	private parseUtils(content: string, filePath: string): UtilInfo[] {
		...
		for (const match of utilMatches) {
			...
			const utilPattern = new RegExp(
				`/\\*\\*[\\s\\S]*?\\*/[\\s\\S]*?export\\s+(?:function\\s+)?${utilName}`,
			);
			const utilMatch = content.match(utilPattern);
			const description = utilMatch
				? this.extractDescription(utilMatch[0])
				: `${utilName} utility function`;
	...
	private parseDesignTokens(content: string, category: string): DesignToken[] {
		...
		for (const pattern of patterns) {
			const matches = content.matchAll(
				new RegExp(pattern.source + '[\\s\\S]*?;', 'g'),
			);
